        ...
        observeEvent(input$detection, {                                                        # "Anomaly Detection!" 버튼을 누루면 아래가 실행됨
            ...
            shiny::withProgress(message = "DB에서 자료를 불러오고 있습니다.", {         # DB에서 불러오는데 시간이 많이 걸리므로 진행바 생성
                ...
                for(i in 1:nrow(df_gennum)) {
                    ...
                    url_elev <- paste0("http://www.gims.go.kr/selectChart1.do?gennum=", 
                                       gennum, "&fdate=", start, "&edate=", end)
                    
                    url_wtemp <- paste0("http://www.gims.go.kr/selectChart2.do?gennum=", 
                                        gennum, "&fdate=", start, "&edate=", end)
                    
                    url_ec <- paste0("http://www.gims.go.kr/selectChart3.do?gennum=", 
                                     gennum, "&fdate=", start, "&edate=", end)
                    
                    url_glev <- paste0("https://www.gims.go.kr/ObsvGetElLev.do?gennum=",
                                       gennum)
                    ...
                    df_real_temp <- data.frame(
                        GENNUM = gennum,
                        YMD = fromJSON(as.character(POST(url_elev, query = requset)))[1][, 1],
                        ELEV = fromJSON(as.character(POST(url_elev, query = requset)))[2][, 1],
                        WTEMP = fromJSON(as.character(POST(url_wtemp, query = requset)))[2][, 1],
                        EC = fromJSON(as.character(POST(url_ec, query = requset)))[2][, 1]) %>% 
                        mutate(LEV = fromJSON(as.character(POST(url_glev, 
                                                                query = requset)))[[1]][1, 1] - 
                                   .[, 3])
