...
ngwServer <- function(id) {
    moduleServer(id, function(input, output, session) {
        observeEvent(input$detection, {               
            ...
            load("R/input/df_real.RData")
            
            df_real <- df_real %>% 
                filter(!response.resultData.ymd == Sys.Date() %>% 
                           str_replace_all("-", "")) # 오늘 것은 확정된 값이 아니어서 제외함
            ...
            shiny::withProgress(message = "DB에서 자료를 불러오고 있습니다.", {       
                ...
                startday <- df_real %>% pull(response.resultData.ymd) %>% 
                    tail(1) %>% ymd() + days(1) # 아래 코드를 이어붙이니 에러가 나서 분리하였음
                startday <- startday %>% as.character() %>% 
                    str_replace_all("-", "")
                ...
                for(i in 1:nrow(df_gennum)) {
                    ...
                    gennum <- df_gennum[i,]
                    startdate <- startday  # 저장해놓은 실시간 자료 이후부터 읽어옴
                    enddate <- today_
            ...
            save(df_real, file = "R/input/df_real.RData")
            load("R/input/df_real.RData")
            ...
            # save(df, file = "R/input/df.rda")  # 앱 보수시 API 읽는 시간 절약을 위해 사용
            # load("R/input/df.rda")
